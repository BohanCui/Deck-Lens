...
└── examples/
    └── spectrum-analyzer.md   # Pitch 模式的完整示例
```
...
`examples/spectrum-analyzer.md` 是一个完整的 Pitch 模式示例——频谱仪软件改进的比赛汇报，按照 Pitch 模式的十个环节逐一展开。可以直接参考它的表达方式和内容深度。
...
- **中文示例** — 目前 `examples/spectrum-analyzer.md` 只有英文版，补一个 `-zh` 版本能帮中文用户更快上手
- **补齐另外三种模式的示例** — 现在只有 Pitch 模式有完整示例，内部汇报、体制内、技术交接各补一个会更好
- **减少三份 SKILL 文件的维护成本** — 现在 `SKILL.md`、`SKILL-en.md`、`SKILL-zh.md` 需要手动保持同步，值得考虑合并成一个主文件 + 独立的翻译文档

### 未来可添加

- **更多设计模板** — 目前只有一套宝蓝色浅色主题，可以再加深色主题、极简学术风等预设
- **混合模式示例** — 比如一个技术型路演，用实例演示"从另一种模式借一个环节"具体怎么操作
- **新增模式** — 仅在确实有需求时考虑，比如教学/培训模式、会议演讲模式
- **模式选择流程图** — 在文档里加一张可视化决策图，方便不想跑 skill 也能自己选模式的人

欢迎提建议或贡献——可以提 issue 或 pull request。
